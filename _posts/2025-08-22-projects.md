...
/* ---------- Kill theme underlines inside accordion ---------- */
/* Some themes draw heading rules via borders, shadows, or bg-images on h3 or the first child */
.role-slideshows .acc-item h3,
.role-slideshows .acc-item h3::before,
.role-slideshows .acc-item h3::after,
.role-slideshows .acc-item .acc-head,
.role-slideshows .acc-item .acc-head .acc-btn{
  border-bottom: none !important;
  box-shadow: none !important;
  background-image: none !important;
}

/* ---------- Single blue underline only when expanded ---------- */
/* Works for both <span class="acc-head">…</span> and <h3>…</h3> wrappers */

/* Prefer :has() for modern browsers */
@supports selector(:has(*)) {
  .role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) .acc-btn{
    border-bottom: 2px solid #3b82f6 !important; /* blue */
  }
}
...
/* When accordion is expanded, color both header bottom + panel top red */
.role-slideshows .acc-btn[aria-expanded="true"] {
