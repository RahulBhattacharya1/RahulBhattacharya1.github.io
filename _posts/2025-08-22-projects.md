...
/* Accordion header wrapper (replacing <h3>) */
.acc-head {
...
.acc-head .acc-btn {
  display: grid;
  grid-template-columns: 1fr auto; /* title left, meta right */
  align-items: center;
  width: 100%;
  padding: .9rem 1rem;
  font-weight: 600;
...
/* --- Make the header a grid so meta can’t overflow --- */
.acc-btn{
  box-sizing:border-box;      /* we used all:unset; restore sane sizing */
  display:grid;
...
/* Right-side tech stack “meta” */
.acc-meta{
  white-space:nowrap;
  overflow:hidden;
  text-overflow:ellipsis;
  max-width: 40%;             /* clamp so it never pushes outside */
}

/* Optional: a bit more room on small screens */
@media (max-width: 768px){
  .acc-meta{ max-width: 55vw; }
}

/* Remove the default underline entirely */
.acc-head,
.acc-head .acc-btn {
...
/* Only show a blue line when expanded */
.acc-head .acc-btn[aria-expanded="true"] {
  border-bottom: 2px solid #3b82f6 !important; /* Tailwind blue-500 */
}

/* When any accordion item is open, kill theme underlines in that item */
.role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) h3,
.role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) h3::before,
.role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) h3::after,
.role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) .acc-head,
.role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) .acc-head .acc-btn {
  border-bottom: 0 !important;
  background-image: none !important;
  box-shadow: none !important;
}

/* Show a single blue rule only when open */
.role-slideshows .acc-item:has(.acc-btn[aria-expanded="true"]) .acc-btn {
